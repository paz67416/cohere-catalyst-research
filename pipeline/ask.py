...
指示書v1をシステムプロンプトとして埋め込み済み。毎回貼る必要はありません。

使い方:
  1. 先に `python ingest.py` を実行しておく
  2. `export CO_API_KEY=あなたのAPIキー`
...
CHAT_MODEL = "command-a-03-2025"   # 理論的検討向けの最上位モデル
TOP_K_SEARCH = 20                  # ベクトル検索で拾う候補数
TOP_N_RERANK = 5                   # Rerank後に根拠として使う数
# -------------------------------------------------------------------------

# 指示書v1（研究コンテキスト）をシステムプロンプトとして埋め込み
...
- 中心仮説: フロー/ゾーン状態には「二重の後性」構造がある。(1)クオリアが形成された後に主体が現れる、(2)「私がやった」という行為の帰属も行為の後から到来する
- 自由意志は行為の開始時点ではなく「構え(kamae)」に位置づける(Libet型の知見への応答)
